...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 cadyn /Users/kevinwinni/Desktop/SingleCell_NEURON/Pyramidal/x86_64/cadyn.mod\n");
...
static int  rate ( _threadargsprotocomma_ double _lv ) {
    if ( _lv < - 57.5 ) {
     ...
     uinf = 1.0 / ( exp ( - ( _lv + vhalf ) / k ) + 1.0 ) ;
     }
   utau = 1.25 * 2.0 / ( exp ( - 0.031 * ( _lv + vhalf ) ) + exp ( 0.031 * ( _lv + vhalf ) ) ) ;
